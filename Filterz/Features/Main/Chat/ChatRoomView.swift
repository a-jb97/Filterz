...
struct ChatRoomView: View {
    ...
    private func shouldShowTimestamp(at index: Int) -> Bool {
        ...
        let calendar = Calendar.current
        let curMinute = calendar.dateComponents([.year, .month, .day, .hour, .minute], from: current.createdAt)
        let nextMinute = calendar.dateComponents([.year, .month, .day, .hour, .minute], from: next.createdAt)
        return curMinute != nextMinute
    }
}
